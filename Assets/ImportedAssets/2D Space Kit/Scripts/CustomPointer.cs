...
public class CustomPointer : MonoBehaviour
{
    ...
    public bool use_mouse_input; //Pointer will be controlled by the mouse.

    public bool use_gamepad_input; //Pointer will be controlled by a joystick

    //public bool use_accelerometer_input = false;	//Pointer will be controlled by accelerometer
    public bool pointer_returns_to_center; //Pointer will drift to the center of the screen (Use this for joysticks)

    public bool
        instant_snapping; //If the pointer returns to the center, this will make it return to the center instantly when input is idle. Only works for joysticks

    public float center_speed = 5f; //How fast the pointer returns to the center.

    public bool invert_y_axis; //Inverts the y axis.


    public float
        deadzone_radius; //Deadzone in the center of the screen where the pointer can move without affecting the ship's movement.
    ...
    private void Update()
    {
        ...
        if (use_mouse_input)
        {
            var x_axis = Input.GetAxis("Mouse X");
            var y_axis = Input.GetAxis("Mouse Y");
