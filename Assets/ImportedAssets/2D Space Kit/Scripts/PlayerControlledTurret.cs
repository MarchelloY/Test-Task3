    ...
    private void Update()
    {
        ...
        transform.rotation = Quaternion.Euler(new Vector3(0, 0,
            Mathf.LerpAngle(transform.rotation.eulerAngles.z,
                Mathf.Atan2(direction.y, direction.x) * Mathf.Rad2Deg - 90f, turret_rotation_speed * Time.deltaTime)));


        if (Input.GetMouseButtonDown(0) && barrel_hardpoints != null)
        {
            var bullet = Instantiate(weapon_prefab, barrel_hardpoints[barrel_index].transform.position,
                transform.rotation);
